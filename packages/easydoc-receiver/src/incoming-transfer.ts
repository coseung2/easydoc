...
export function safeFilename(input: string): string {
  const name = input.trim().replace(/[ .]+$/u, '')
  if (
    !name ||
    name === '.' ||
    name === '..' ||
  ...
  if (WINDOWS_RESERVED_STEMS.has(stem)) throw new Error('invalid_filename')
  return name
}
...
export class IncomingTransfer {
  ...
  static async create(
    transfer: TransferStartMessage,
    rootDir: string,
    freeSpaceCheck: FreeSpaceCheck = defaultFreeSpaceCheck,
  ...
  static async resume(transferId: string, rootDir: string): Promise<IncomingTransfer> {
    ...
    try {
      metadata = JSON.parse(await readFile(metadataPath, 'utf8')) as ReceiveMetadata
    } catch {
    ...
    return new IncomingTransfer(
      ...
      partPath,
      metadata,
      await open(partPath, 'r+'),
    )
  }

  get transfer(): TransferStartMessage {
    return this.metadata.transfer
  }

  get resumeFromChunk(): number {
    return this.metadata.nextChunk
  }

  get receivedThroughChunk(): number {
    return this.metadata.nextChunk - 1
  }

  get bytesWritten(): number {
    return this.metadata.bytesWritten
  }
  ...
  async writeChunk(index: number, payload: Uint8Array): Promise<ReceiveProgress> {
    ...
    if (index < this.metadata.nextChunk) {
      return {
    ...
    if (this.metadata.bytesWritten + payload.byteLength > this.metadata.transfer.size) {
  ...
  private async finalize(): Promise<ReceiveProgress> {
    ...
    if ((await sha256File(this.partPath)) !== this.metadata.transfer.sha256.toLowerCase()) {
