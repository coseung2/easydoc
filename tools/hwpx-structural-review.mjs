...
function parseXml(xml, name) {
  ...
  parser.on('doctype', () => { throw new Error(`${name}: DOCTYPE is not allowed`) })
  parser.on('error', (error) => { throw new Error(`${name}: ${error.message}`) })
  parser.on('opentag', (tag) => {
    if (nodes.length >= 100_000 || stack.length >= 128) throw new Error(`${name}: XML complexity limit`)
    const node = {
      local: tag.local,
      uri: tag.uri,
...
export function reviewHwpx(bytes) {
  ...
  try {
    if (!(bytes instanceof Uint8Array) || bytes.byteLength > MAX_ARCHIVE_BYTES || bytes.byteLength < 38) {
    ...
    const parts = unzipSync(bytes, {
      filter(part) {
        ...
        names.add(part.name)
        if (names.size > MAX_PARTS || totalBytes > MAX_TOTAL_BYTES || part.originalSize > MAX_PART_BYTES) {
          throw new Error('Uncompressed package limit exceeded')
        }
        if (part.name.startsWith('/') || part.name.includes('\\') || part.name.split('/').some((p) => p === '..')) {
    ...
    if (!parts.mimetype || strFromU8(parts.mimetype) !== 'application/hwp+zip') fail('Incorrect HWPX mimetype')
    const required = ['version.xml', 'settings.xml', 'META-INF/container.xml', 'META-INF/manifest.xml', 'Contents/content.hpf', 'Contents/header.xml', 'Contents/section0.xml']
    for (const name of required) if (!parts[name]) fail(`Missing package part: ${name}`)
    if (errors.length) return result
    const documents = new Map()
    for (const [name, data] of Object.entries(parts)) {
      if (/\.(xml|hpf)$/i.test(name)) documents.set(name, parseXml(new TextDecoder('utf-8', { fatal: true }).decode(data), name))
    }
    ...
    if (Number(header.root.attrs.secCnt) !== sections.length) fail('Header section count does not match package sections')
    const manifest = packageDoc.nodes.filter((n) => n.uri === OPF && n.local === 'item')
    const manifestIds = new Map()
    for (const item of manifest) {
      if (!item.attrs.id || manifestIds.has(item.attrs.id)) fail(`Duplicate/empty manifest id: ${item.attrs.id}`)
    ...
    const idSets = Object.fromEntries(['charPr', 'paraPr', 'style', 'borderFill', 'tabPr'].map((kind) => {
    ...
    for (const [name, document] of documents) {
      for (const node of document.nodes) {
        for (const kind of ['charPr', 'paraPr', 'style', 'borderFill', 'tabPr']) {
          ...
          if (ref !== undefined && !idSets[kind].has(ref)) fail(`${name}: dangling ${kind}IDRef=${ref}`)
        }
        if (node.attrs.binaryItemIDRef !== undefined && !manifestIds.has(node.attrs.binaryItemIDRef)) {
    ...
    for (const [name, section] of sections) {
      ...
      if (!page || !(Number(page.attrs.width) > 0) || !(Number(page.attrs.height) > 0)) fail(`${name}: explicit page dimensions are missing`)
      if (!section.nodes.some((n) => n.uri === HP && n.local === 'p')) fail(`${name}: section has no paragraph`)
      for (const table of section.nodes.filter((n) => n.uri === HP && n.local === 'tbl')) {
        ...
        for (const [r, row] of rows.entries()) {
          ...
          for (const [c, cell] of cells.entries()) {
            ...
            if (!address || Number(address.attrs.rowAddr) !== r || Number(address.attrs.colAddr) !== c) fail(`${name}: missing/incorrect cell address at ${r},${c}`)
            if (!span || !(Number(span.attrs.rowSpan) >= 1) || !(Number(span.attrs.colSpan) >= 1)) fail(`${name}: missing cell span at ${r},${c}`)
            if (!subList || !subList.children.some((n) => n.local === 'p')) fail(`${name}: missing cell paragraph at ${r},${c}`)
          }
        }
      }
    }
