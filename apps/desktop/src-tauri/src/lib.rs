...
struct AppState {
    ...
    receiver_task: Mutex<Option<tauri::async_runtime::JoinHandle<()>>>,
}
        ...
        let settings = read_json(&settings_path())
            ...
            .unwrap_or(DesktopSettings {
                ...
                relay_base_url: std::env::var("EASYDOC_RELAY_URL")
                    .unwrap_or_else(|_| "https://easydoc-relay.example.workers.dev".into()),
                paired: false,
                connected: false,
            });
...
#[tauri::command]
async fn create_pairing(state: State<'_, Arc<AppState>>) -> Result<PairingView, String> {
    let settings = state.settings.lock().unwrap().clone();
    let existing = state.pairing.lock().unwrap().clone();
    let device_id = existing
        .as_ref()
        .map(|p| p.device_id.clone())
        .unwrap_or_else(|| format!("desktop_{}", Uuid::new_v4()));
    let public_key = match existing {
        Some(pairing) if load_credential("device-private", &device_id).is_ok() => {
            pairing.public_key
        }
        _ => generate_identity(&device_id)?,
    };
    ...
    store_credential(
        "pairing-bootstrap",
        &issue.pairing.room_id,
        &issue.desktop_secret,
    )?;
    let pairing_state = PairingState {
        device_id: issue.pairing.desktop_id.clone(),
        room_id: issue.pairing.room_id.clone(),
        public_key: issue.pairing.public_key.clone(),
    };
    ...
    *state.pairing.lock().unwrap() = Some(pairing_state);
    ...
    Ok(PairingView {
        qr_payload: serde_json::to_string(&issue.pairing).map_err(|e| e.to_string())?,
        expires_at: issue.pairing.expires_at,
    })
}
...
async fn receiver_loop(
        ...
        match message.map_err(|e| e.to_string())? {
            Message::Text(text) => {
                ...
                }) = serde_json::from_str::<Control>(&text)
                {
                    ...
                    let part_path = root.join(format!("{}.part", safe_name));
                ...
                if let Some(receive) = current.as_mut() {
                    ...
                    if receive.written == receive.expected_size {
                        ...
                        let snapshot = {
                            ...
                            inbox.insert(0, item.clone());
                            inbox.clone()
                        };
                        write_json(&inbox_path(), &snapshot).await?;
    ...
    state.settings.lock().unwrap().connected = false;
    Ok(())
}

fn start_receiver_supervisor(app: tauri::AppHandle, state: Arc<AppState>) {
    if state.receiver_task.lock().unwrap().is_some() {
        return;
    }
    ...
    let task = tauri::async_runtime::spawn(async move {
        loop {
            ...
            let pairing = task_state.pairing.lock().unwrap().clone();
            if let Some(pairing) = pairing {
                let _ = receiver_loop(app.clone(), task_state.clone(), settings, pairing).await;
                task_state.settings.lock().unwrap().connected = false;
            }
            tokio::time::sleep(std::time::Duration::from_secs(3)).await;
        }
    });
    *state.receiver_task.lock().unwrap() = Some(task);
}
...
async fn connect_receiver(
    ...
    if state.pairing.lock().unwrap().is_none() {
...
async fn delete_file(state: State<'_, Arc<AppState>>, path: String) -> Result<(), String> {
    ...
    let snapshot = {
        let mut inbox = state.inbox.lock().unwrap();
        inbox.retain(|item| item.path != path);
        inbox.clone()
    };
    write_json(&inbox_path(), &snapshot).await
}
...
pub fn run() {
    let (settings, pairing, inbox) = load_initial();
