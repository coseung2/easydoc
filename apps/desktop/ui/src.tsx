...
function App() {
  ...
  useEffect(() => { refresh().catch((error) => setMessage(String(error))); const id = setInterval(() => refresh().catch(() => undefined), 3000); return () => clearInterval(id); }, []);

  const createPairing = async () => { setMessage(""); try { setPairing(await invoke<Pairing>("create_pairing")); } catch (error) { setMessage(String(error)); } };
  ...
  return <main className="shell">
    ...
    {pairing && <section className="pairing"><div><strong>휴대폰에서 QR 스캔</strong><p>5분 동안 한 번만 사용할 수 있습니다.</p><code>{pairing.qrPayload}</code></div><button className="secondary" onClick={() => setPairing(null)}>닫기</button></section>}
    <section className="inbox"><div className="section-title"><h2>받은 파일</h2><span>{items.length}개</span></div>{items.length === 0 ? <div className="empty"><div className="empty-icon">↓</div><strong>아직 받은 파일이 없습니다</strong><p>휴대폰에서 문서를 스캔하고 이 PC로 보내세요.</p></div> : items.map((item) => <article key={`${item.path}-${item.arrivedAt}`}><div className="file-icon">PDF</div><div className="file-info"><strong>{item.filename}</strong><p>{sizeLabel(item.size)} · {timeLabel(item.arrivedAt)}</p></div><span className={`status ${item.status}`}>{item.status}</span><button className="ghost" onClick={() => invoke("open_path", { path: item.path })}>열기</button><button className="ghost" onClick={() => invoke("reveal_path", { path: item.path })}>폴더</button></article>)}</section>
