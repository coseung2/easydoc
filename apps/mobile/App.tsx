...
import { initializeDocumentScannerSdk } from "./src/scanner/scanbot-sdk";
...
export default function App() {
  ...
  useEffect(() => {
    void initializeDocumentScannerSdk().catch(() => {});
    void loadPairings();
    void loadDocuments();
  ...
  return <SafeAreaProvider><SafeAreaView style={[styles.safe, cameraRoute && styles.cameraSafe]} edges={cameraRoute ? [] : ["top", "right", "bottom", "left"]}>
    ...
    {route === "scan" && <ScannerScreen onClose={() => setRoute("home")} onSaved={async (document) => { addLocalDocument(document); const target = selectedPairing; if (target) { try { await enqueueForTarget(toRecentFile(document), target); } catch (error) { setTransferError(errorMessage(error, "파일은 저장했지만 PC 전송 목록에 추가하지 못했습니다.")); } } }} onFinished={() => setRoute("documents")} />}
...
function Home({ recent, pairings, selectedPairing, onlineByPairing, error, onRetry, onSelectPc, onScan, onOpen, onOpenRecent, onSearch, onDocuments, onPresentation, onOcr, onTools, onPair }: { recent: RecentFile[]; pairings: MobilePairingForUi[]; selectedPairing: MobilePairingForUi | null; onlineByPairing: Readonly<Record<string, boolean>>; error: string; onRetry: () => void | Promise<void>; onSelectPc: (pairing: StoredMobilePairing) => void | Promise<void>; onScan: () => void; onOpen: () => void; onOpenRecent: (file: RecentFile) => void; onSearch: () => void; onDocuments: () => void; onPresentation: () => void; onOcr: () => void; onTools: () => void; onPair: () => void }) {
  ...
  return <ScrollView style={sharedStyles.content} contentContainerStyle={styles.homeContent} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.pcCard}><View style={styles.pcCardHeader}><View style={styles.pcCardTitleBlock}><Text style={styles.pcTitle}>PC로 바로 보내기</Text><Text style={styles.pcMeta}>선택한 PC로 새 촬영·스캔 파일을 전송합니다.</Text></View><Feather name="monitor" size={22} color={colors.primary} /></View>
