  ...
  return (
    <View style={styles.nav}>
      {tabs.map((tab) => {
        ...
        return (
          <Pressable key={tab.key} style={styles.navItem} onPress={() => onChange(tab.key)} accessibilityRole="button">
...
export function SearchBar({ label = "검색" }: { label?: string }) {
  return <View style={styles.search}><Feather name="search" size={17} color={colors.textMuted} /><Text style={styles.searchText}>{label}</Text></View>;
}
...
export function IconButton({ icon, onPress }: { icon: ComponentProps<typeof Feather>["name"]; onPress?: () => void }) {
  return <Pressable onPress={onPress} style={styles.iconButton}><Feather name={icon} size={19} color={colors.text} /></Pressable>;
}
...
const styles = StyleSheet.create({
  nav: { height: 66, marginHorizontal: 20, borderTopWidth: 1, borderTopColor: colors.border, flexDirection: "row", justifyContent: "space-between", backgroundColor: colors.background },
  ...
  chip: { height: 28, paddingHorizontal: 12, borderRadius: radius.pill, borderWidth: 1, borderColor: colors.border, justifyContent: "center", backgroundColor: colors.surface },
